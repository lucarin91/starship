...
starship_precmd() {
    ...
    if [[ $STARSHIP_START_TIME ]]; then
        ...
        PS1="$(::STARSHIP:: prompt --status=$STATUS --jobs="$(jobs -p | wc -l)" --cmd-duration=$STARSHIP_DURATION --history="$(echo $(history 5 | cut -c 8-))")"
        unset STARSHIP_START_TIME
    else
        PS1="$(::STARSHIP:: prompt --status=$STATUS --jobs="$(jobs -p | wc -l)" --history="$(echo $(history 5 | cut -c 8-))")"
    fi
